...
export default function PublishConfirmModal({
  ...
  // 同步计算风险项（替代 useEffect + setState 模式）
  const risks = useMemo(() => {
    ...
    if (!account) items.push({ level: 'high', text: '未选择公众号账号，请先选择发布账号。' });
      ...
      items.push({ level: 'high', text: '当前公众号账号未登录，请先到设置页扫码。' });
    if (!title.trim()) items.push({ level: 'high', text: '标题为空，发布前需要补充标题。' });
    if (title.trim().length > 64)
      items.push({ level: 'medium', text: '标题较长，建议压缩到 64 字以内。' });
    if (!content.trim())
      items.push({ level: 'medium', text: '正文为空，保存草稿可以继续，但直接发布风险较高。' });
    if (!cover) items.push({ level: 'medium', text: '未选择封面，建议补充封面后再发布。' });
    if (images.length === 0)
      items.push({
        level: action === 'publish' ? 'high' : 'medium',
        text: '未附带图片，请确认这是否符合本次内容。',
      });
    else if (images.length < 3)
      items.push({ level: 'low', text: '图片数量低于建议值，图集内容建议至少 3 张。' });
    if (images.some(riskText) || (cover && riskText(cover)))
      items.push({ level: 'medium', text: '存在疑似水印或来源标识图片，请人工确认。' });

    // 标题 emoji 检测（微信公众平台不支持标题 emoji）
    if (
      // eslint-disable-next-line no-misleading-character-class
      /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{FE00}-\u{FE0F}\u{200D}]/u.test(
        title,
      )
    ) {
      items.push({
        level: 'medium',
        text: '标题包含 emoji 表情，微信公众号可能不支持显示，建议去除。',
      });
    }
  ...
  useEffect(() => {
    ...
    // 此处 setState 用于异步 debounce 场景，不适合 useMemo 替代
    // eslint-disable-next-line react-hooks/set-state-in-effect
  }, [title]);
  ...
  const allRisks = dupCheck?.duplicate
    ? [
        ...risks,
        {
          ...
          text: `标题"${title}"与已有内容相似，建议修改以避免重复发布。`,
        },
      ]
    : risks;
  ...
  return (
    <Modal
      open={open}
      onClose={onCancel}
      className="w-[760px] max-w-[calc(100vw-32px)] max-h-[88vh] overflow-y-auto"
    >
      <div className="flex items-start justify-between gap-4 mb-5">
        ...
        <span
          className={`px-2.5 py-1 rounded-full text-xs font-semibold ${action === 'publish' ? 'bg-danger/10 text-danger' : 'bg-accent-soft text-accent'}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-5">
        <div className="space-y-3">
          <div className="rounded-lg border border-border bg-bg-secondary p-3">
            ...
            <div className={`text-xs mt-1 ${account?.logged_in ? 'text-success' : 'text-danger'}`}>
              {account?.logged_in ? '已登录' : '未登录'}
            </div>
          </div>
          {cover ? (
            <button
              className="block w-full rounded-lg overflow-hidden border border-border bg-bg-secondary"
            ...
            <div className="rounded-lg border border-dashed border-border p-6 text-sm text-text-muted text-center">
              未选择封面
            </div>
          )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="text-base font-semibold text-text leading-snug break-words">
              {title || '无标题'}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            {allRisks.length === 0 ? (
              <div className="rounded-lg bg-success/10 text-success border border-success/20 px-3 py-2 text-sm">
                未发现明显风险，仍建议人工确认内容无误。
              </div>
            ) : (
          ...
          {highRisk && (
            <Checkbox checked={ackHighRisk} onChange={setAckHighRisk}>
              我已确认以上高风险项，仍要继续
            </Checkbox>
          )}
          {action === 'draft' && (
            <Checkbox checked={headless} onChange={setHeadless}>
              后台运行（无浏览器窗口，不可扫码登录）
            </Checkbox>
          )}
        </div>
      ...
      <div className="flex justify-end gap-2.5 mt-6 pt-4 border-t border-border-subtle">
        ...
        <button
          className={`btn ${action === 'publish' ? 'btn-danger' : 'btn-primary'}`}
          onClick={() => onConfirm(headless)}
          disabled={!canSubmit}
        >
          {loading ? (
            <>
