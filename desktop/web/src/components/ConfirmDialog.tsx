    ...
    <div className="fixed inset-0 z-[9000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in" onClick={onCancel}>
      <div className="bg-bg-card border border-border rounded-2xl p-6 min-w-[320px] max-w-[400px] shadow-xl animate-scale" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2.5 justify-end">
          <button className="btn" onClick={onCancel} disabled={localLoading}>{cancelText}</button>
          <button className={`btn ${danger ? 'btn-danger' : 'btn-primary'}`} onClick={handleConfirm} disabled={localLoading}>
            {localLoading ? <><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> 处理中</> : confirmText}
          </button>
        </div>
      </div>
    </div>
