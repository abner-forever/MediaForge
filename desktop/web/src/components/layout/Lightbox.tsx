  ...
  return (
    <div className="fixed inset-0 z-[9000] flex flex-col items-center justify-center animate-in">
      ...
      <button
        ...
        className="absolute top-4 right-4 z-20 w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all hover:scale-105"
      >
        <svg
      ...
      {/* 主图区域 — 点击空白处关闭 */}
      <div
        className="relative z-10 flex flex-col items-center justify-center w-full h-full px-16 pb-4 pt-12"
        onClick={closeLightbox}
      >
        <div className="relative flex items-center justify-center w-full flex-1 min-h-0">
          {showNav && (
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all hover:scale-105 backdrop-blur"
            >
              <svg
          ...
          <img
            src={url}
            alt=""
            className={`max-h-full max-w-full object-contain select-none rounded-lg shadow-2xl transition-all duration-700 ease-out ${
          ...
          {showNav && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all hover:scale-105 backdrop-blur"
            >
              <svg
                className="w-5 h-5 text-white"
                viewBox="0 0 24 24"
        ...
        {showNav && (
          <div
            className="flex flex-col items-center w-full max-w-[min(90vw,900px)] mt-3 pb-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              ref={thumbsRef}
