...
export default function ContextMenu({ items, position, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const x =
      position.x + r.width > window.innerWidth ? window.innerWidth - r.width - 8 : position.x;
    const y =
      position.y + r.height > window.innerHeight ? window.innerHeight - r.height - 8 : position.y;
  ...
  return (
    <div
      ref={ref}
      className="fixed z-[9500] min-w-[170px] py-1.5 bg-bg-card border border-border rounded-xl shadow-xl animate-scale"
