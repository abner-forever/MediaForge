...
export default function Checkbox({ checked, onChange, disabled, indeterminate, className, children }: CheckboxProps) {
  const uid = useId();
  return (
    <label
      className={`checkbox-root${disabled ? ' checkbox-disabled' : ''}${className ? ` ${className}` : ''}`}
      onPointerDown={e => { if (disabled) e.preventDefault(); }}
    >
      <span className={`checkbox-target${checked ? ' checkbox-checked' : ''}${indeterminate ? ' checkbox-indeterminate' : ''}`}>
        <input
          type="checkbox"
          checked={checked}
        ...
        {indeterminate && !checked && (
          <span className="checkbox-indet" aria-hidden />
        )}
      </span>
