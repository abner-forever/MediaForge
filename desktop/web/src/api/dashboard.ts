...
export const dashboardApi = {
  ...
  operations: (page = 1, pageSize = 10) =>
    get<OperationsResponse>(`/api/dashboard/operations?page=${page}&page_size=${pageSize}`),
  deleteOperations: (ids: string[]) =>
    post<{ success: boolean; deleted: number }>('/api/dashboard/operations/delete', { ids }),
  clearOperations: () =>
    post<{ success: boolean; deleted: number }>('/api/dashboard/operations/delete', {
      clear: true,
    }),
};
