...
export const userApi = {
  /** 发送验证码 */
  sendCode: (email: string) => post<SendCodeResponse>('/api/user/send-code', { email }),

  /** 用户注册 */
  register: (data: RegisterRequest) => post<RegisterResponse>('/api/user/register', data),

  /** 用户登录 */
  login: (data: LoginRequest) => post<LoginResponse>('/api/user/login', data),
  ...
  /** 获取当前用户信息 */
  getCurrentUser: () => get<{ success: boolean; data: UserProfile }>('/api/user/current'),

  /** 获取用户资料 */
  getProfile: () => get<{ success: boolean; data: UserProfile }>('/api/user/profile'),
  ...
  /** 获取用户设置 */
  getSettings: () => get<{ success: boolean; data: Record<string, unknown> }>('/api/user/settings'),
  ...
  /** 获取绑定设备列表 */
  getDevices: () => get<{ success: boolean; data: string[] }>('/api/user/devices'),
  ...
  getSavedToken: () => get<{ success: boolean; token: string | null }>('/api/user/saved-token'),
