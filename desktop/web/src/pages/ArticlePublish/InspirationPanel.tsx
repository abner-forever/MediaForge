...
export default function InspirationPanel({
  inspirationExpanded, inspirationKeyword, inspirationResults, inspirationLoading,
  onToggle, onKeywordChange, onSearch, onPickTopic,
}: {
  inspirationExpanded: boolean; inspirationKeyword: string; inspirationResults: InspirationTopic[]; inspirationLoading: boolean;
  onToggle: () => void; onKeywordChange: (v: string) => void; onSearch: () => void; onPickTopic: (topic: InspirationTopic) => void;
}) {
  return (
          ...
          {inspirationResults.length > 0 && (
            <div style={{ marginTop: 8, maxHeight: 200, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
              {inspirationResults.map((t, i) => (
                <div key={i} onClick={() => onPickTopic(t)} style={{
