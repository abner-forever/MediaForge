              ...
              {filteredIndices.length > 0 && (
                <div className="flex items-center justify-center gap-4 pt-4 border-t border-border mt-4">
                  ...
                  <button className="btn" onClick={loadMore} disabled={searching}>
                    {searching ? <><span className="w-3 h-3 border-2 border-text-muted/30 border-t-accent rounded-full animate-spin" /> 加载中</> : '加载更多'}
                  </button>
                </div>
              )}
            </>
          ...
          {activeTab === 'gallery' && allLocalImages.length > 0 && (
            <>
              <div className="flex items-center gap-2 mb-4">
                ...
                <button className="btn btn-xs ml-auto" onClick={() => selectAllImages(allLocalImages.map(i => i.path))}>全选/取消</button>
