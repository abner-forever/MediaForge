...
export default function UploadModal({
  ...
  const startUpload = async () => {
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      setItems((prev) =>
        prev.map((item, j) => (j === idx ? { ...item, status: 'uploading' } : item)),
      );
      try {
        await materialsApi.upload(pending[i].file, currentPath);
        setItems((prev) =>
          prev.map((item, j) => (j === idx ? { ...item, status: 'success' } : item)),
        );
      } catch (e: any) {
        setItems((prev) =>
          prev.map((item, j) =>
            j === idx ? { ...item, status: 'error', error: e.message } : item,
          ),
        );
      }
    }
  ...
  return (
    ...
    >
      <div
        className="w-[520px] max-h-[80vh] bg-bg-card rounded-2xl shadow-2xl flex flex-col overflow-hidden"
      ...
      >
        ...
        <div className="p-5 space-y-4 overflow-auto">
          ...
          >
            ...
            <p className="text-xs text-text-muted/60 mt-1">
              支持 JPG / PNG / WebP / GIF / MD / TXT / PDF
            </p>
          </div>
            ...
            <div className="space-y-2">
              ...
              {items.map((item, index) => (
                ...
                >
                  ...
                  {item.status === 'pending' && (
                    <button
                      className="shrink-0 p-1 rounded hover:bg-bg-hover text-text-muted hover:text-text"
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-3 border-t border-border shrink-0">
          ...
          >
            {uploading
              ? '上传中...'
              : `上传 (${items.filter((i) => i.status === 'pending').length})`}
          </button>
        </div>
      </div>
    </div>
