...
export default function FileCard({
  file, selected, onToggleSelect, onOpenPreview, onContextMenu,
}: {
  file: BrowseFile;
  selected: boolean;
  onToggleSelect: () => void;
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 opacity-0 group-hover/file:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
      ...
      <div className="px-2.5 py-1.5">
        <div className="text-[11px] text-text whitespace-nowrap overflow-hidden text-ellipsis">{file.name}</div>
        <div className="text-[9px] text-text-muted/60 tabular-nums mt-0.5">{formatSize(file.size)}</div>
      </div>
    </div>
  );
}
