    ...
    <div className="space-y-6 animate-in">
      <div className="flex items-start justify-between">
        ...
        <HelpGuide title="发布队列 — 使用说明">
          <p>
            <b>1. 队列概览</b>
            ：所有待发布内容按时间排列，每张卡片显示标题、封面图、图片数量和当前状态。
          </p>
          <p>
            <b>2. 筛选与排序</b>
            ：顶部可按公众号账号、内容类型（图文/文章）、状态（待发布/已发布/失败）筛选。
          </p>
          <p>
            <b>3. 预览内容</b>：展开卡片可预览完整标题、图片列表和文章摘要，确认无误后再发布。
          </p>
          <p>
            <b>4. 发布操作</b>
            ：点击卡片上的「发布」按钮将内容推送到绑定的公众号。发布过程中请勿关闭程序。
          </p>
          <p>
            <b>5. 效果录入</b>：已发布的项目可录入阅读量、点赞、评论等数据，用于后续数据分析。
          </p>
          <p>
            <b>6. 编辑与删除</b>：可编辑队列项的标题和图片顺序，或将其从队列中移除。
          </p>
        </HelpGuide>
      </div>
      ...
      {accounts.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="w-28">
            <Select size="sm" value={filterType} onChange={setFilterType} options={typeOptions} />
          </div>
          <div className="w-32">
            <Select
              size="sm"
        ...
        <div className="card">
          <div className="empty-state py-16">
            ...
            <div className="empty-state-title">
              {queue.length === 0 ? '发布队列为空' : '没有匹配的项'}
            </div>
            <div className="empty-state-desc">
              {queue.length === 0 ? '请在「图片发现」页面选图并加入发布队列' : '试试调整筛选条件'}
            </div>
          </div>
        </div>
      ) : (
              ...
              {sorted.map((item, idx) => (
                <div key={item.id} className="flex gap-4">
                  ...
                  <div className={`flex-1 min-w-0 ${idx < sorted.length - 1 ? 'pb-7' : ''}`}>
                    <div className="text-[11px] text-text-muted/60 leading-none mb-2 mt-1.5">
