...
export default function SummaryPanel({
  ...
  const successRate = summary.total_posts > 0 ? Math.round((successCount / summary.total_posts) * 100) : 0;
    ...
    <div className="card space-y-5">
      ...
      <div className="flex items-center gap-4">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-bold
          ${failedCount > 0 && successCount > 0 ? 'bg-yellow-500/15 text-yellow-500' :
            failedCount > 0 ? 'bg-red-500/15 text-red-500' :
        ...
        <div>
          <div className="text-lg font-bold text-text">
            {failedCount > 0 && successCount > 0 ? '部分完成' :
             failedCount > 0 ? '运行失败' : '运行成功'}
          </div>
          <div className="text-xs text-text-muted mt-0.5">
            成功率 {successRate}% &middot; {summary.total_posts} 个帖子 &middot; {summary.total_images} 张图片
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="bg-bg-secondary/70 rounded-xl p-3.5 text-center border border-border/30">
          <div className="text-xl font-bold text-green-500 tabular-nums">{summary.total_images}</div>
        ...
        <div className="bg-bg-secondary/70 rounded-xl p-3.5 text-center border border-border/30">
          <div className={`text-xl font-bold tabular-nums ${failedCount > 0 ? 'text-red-500' : 'text-text-muted'}`}>{failedCount}</div>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-xs text-text-muted bg-bg-secondary/30 rounded-xl px-4 py-3 border border-border/30">
        <span className="flex items-center gap-1.5">
          <svg className="w-3.5 h-3.5 text-text-muted/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
        ...
        {hasTokens && (
          <span className="flex items-center gap-1.5">
            <svg className="w-3.5 h-3.5 text-text-muted/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="4 7 4 4 20 4 20 7" /><line x1="9" y1="20" x2="15" y2="20" /><line x1="12" y1="4" x2="12" y2="20" /></svg>
        ...
        <div>
          ...
          <div className="divide-y divide-border/50 rounded-xl border border-border/30 overflow-hidden">
            {summary.items.map((item, i) => (
              <div key={i} className={`flex items-center justify-between py-2.5 px-3.5 text-sm transition-colors
...
function StatusLabel({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${colors[status] || 'text-text-muted bg-bg-secondary'}`}>
