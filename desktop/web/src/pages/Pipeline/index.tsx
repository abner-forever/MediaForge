...
export default function PipelinePage() {
  ...
  const [detailEvents, setDetailEvents] = useState<Array<{
    ts: string;
    event: string;
  ...
  const [detailTokens, setDetailTokens] = useState<{ prompt: number; completion: number } | null>(
    null,
  );
  ...
  const openDetail = useCallback(async (run: RunInfo) => {
    ...
    try {
      ...
      if (p?.prompt_tokens || p?.completion_tokens) {
        setDetailTokens({
          prompt: Number(p.prompt_tokens ?? 0),
          completion: Number(p.completion_tokens ?? 0),
        });
      }
    } catch {
      setDetailEvents([]);
  ...
  return (
    <div className="py-6 px-4">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <div className="flex items-start justify-between">
            ...
            <HelpGuide title="智能流水线 — 使用说明">
              <p>
                <b>1. 配置参数</b>
                ：左侧面板设置平台、搜索模式、艺人/关键词等参数，与「图片发现」页面类似。
              </p>
              <p>
                <b>2. 运行流水线</b>：点击「开始运行」后，系统自动执行 7 个步骤：健康检查 → 抓取帖子
                → 下载图片 → AI 评分 → 生成内容 → 加入队列 → 发布。
              </p>
              <p>
                <b>3. 实时进度</b>
                ：运行过程中可查看当前步骤、进度百分比和详细日志。每个步骤完成后自动进入下一步。
              </p>
              <p>
                <b>4. 决策节点</b>
                ：流水线可能在关键节点暂停等待确认（如选择封面图），请留意黄色提示并做出选择。
              </p>
              <p>
                <b>5. 查看结果</b>
                ：运行完成后展示摘要（下载数、评分、生成文章数），可直接跳转到队列页查看结果。
              </p>
              <p>
                <b>6. 运行历史</b>：下方「运行历史」记录每次流水线的执行情况，方便回溯和对比。
              </p>
            </HelpGuide>
          </div>
                  ...
                  <div key={step.id} className="flex items-start gap-0 flex-1 min-w-0">
                    ...
                    <div className="group relative flex-1 min-w-0">
                      ...
                      >
                        ...
                        <span className="text-[11px] font-medium text-text leading-tight line-clamp-1">
                          {step.label}
                        </span>
                      </div>
                      {/* Tooltip */}
                    ...
                    {i < PIPELINE_STEPS.length - 1 && (
                      <div className="flex items-center pt-3.5 px-0.5 shrink-0">
                        <svg
                          className="w-4 h-4 text-accent/25"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
              ...
              <div className="text-center py-12 px-8">
                ...
                <div className="grid grid-cols-3 gap-4 max-w-2xl mx-auto text-left">
                  {[
                    ...
                    <div
                      key={item.step}
                      className="bg-bg-secondary/50 rounded-xl p-4 border border-border/40 hover:border-accent/20 hover:bg-accent/5 transition-all duration-200"
                    >
          ...
          <div className="w-80">
            <div className="card p-0 overflow-hidden sticky top-6">
              <div className="flex items-center justify-between px-4 pt-3 pb-2">
                ...
                <button
                  onClick={loadHistory}
                  className="text-xs text-text-muted hover:text-text p-1 rounded hover:bg-bg-secondary"
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3 py-3 rounded-xl transition-all group ${
                          isActive ? 'bg-accent/10 ring-1 ring-accent/20' : 'hover:bg-bg-secondary'
                        }`}
                      >
                        {/* 第一行：状态点 + 标题 + 操作 */}
                        <div className="flex items-center gap-2 mb-1.5">
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${statusColor(run.status)}`}
                          />
                          <div className="text-sm font-medium truncate text-text flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          <div className="bg-bg-secondary/60 rounded-lg px-2 py-1.5 text-center">
                            <div className="text-xs font-semibold text-text tabular-nums">
                              {run.total_posts ?? run.processed}
                            </div>
                            <div className="text-[10px] text-text-muted leading-tight">帖子</div>
                          </div>
                          <div className="bg-bg-secondary/60 rounded-lg px-2 py-1.5 text-center">
                            <div className="text-xs font-semibold text-green-600 tabular-nums">
                              {run.published ?? run.processed - run.failed}
                            </div>
                            <div className="text-[10px] text-text-muted leading-tight">入队</div>
                          </div>
                          <div className="bg-bg-secondary/60 rounded-lg px-2 py-1.5 text-center">
                            <div
                              className={`text-xs font-semibold tabular-nums ${run.failed > 0 ? 'text-red-500' : 'text-text-muted'}`}
                            >
                              {run.failed}
                            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-bg-card rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[80vh] flex flex-col">
                ...
                {checkpoint.items.map((item, i) => {
                  ...
                  const imgSrc = (path: string) =>
                    path.startsWith('http')
                      ...
                      : `/images/${encodeURIComponent(path).replace(/%2F/g, '/')}`;
            ...
            <div className="flex gap-3 justify-end px-6 py-3 border-t border-border">
              <button onClick={handleCheckpointDeny} className="btn btn-ghost">
                取消发布
              </button>
              <button onClick={handleCheckpointConfirm} className="btn btn-primary">
                确认发布
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-bg-card rounded-2xl shadow-xl w-full max-w-md mx-4">
            ...
            <div className="px-6 py-4 space-y-2">
              {(() => {
                const posts = decisionReq.context?.posts as
                  | Array<{
                      index: number;
                      text_preview: string;
                      celebrity?: string;
                ...
                return posts && posts.length > 0 ? (
                  <div className="max-h-48 overflow-y-auto space-y-1.5 mb-3 border border-border rounded-xl p-2 bg-bg-secondary/30">
                    {posts.map((post) => (
                      <div
                        key={post.index}
                        className="flex items-start gap-2 p-1.5 rounded-lg hover:bg-bg-secondary text-xs"
                      >
                        <span className="text-text-muted/50 font-mono w-5 shrink-0 text-right">
                          #{post.index}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 text-text-muted">
                            {post.celebrity && (
                              <span className="font-medium text-text">{post.celebrity}</span>
                            )}
                            {post.image_count !== undefined && <span>{post.image_count} 图</span>}
                          </div>
                          <div className="text-text-muted truncate mt-0.5">
                            {post.text_preview || '无文本'}
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {detailRun && (
        ...
        >
          <div
            className="bg-bg-card rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[85vh] flex flex-col"
          ...
          >
            <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-border">
              ...
              <button
                onClick={closeDetail}
                className="p-1.5 rounded-lg hover:bg-bg-secondary text-text-muted"
              >
                <svg
                  className="w-5 h-5"
                  viewBox="0 0 24 24"
              ...
              {detailLoading ? (
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    <StatBox
                      label="帖子"
                      value={detailRun.total_posts ?? detailRun.processed}
                      color="text-accent"
                    />
                    <StatBox
                      label="入队"
                      value={detailRun.published ?? detailRun.processed - detailRun.failed}
                      color="text-green-500"
                    />
                    <StatBox label="失败" value={detailRun.failed} color="text-red-500" />
                    <StatBox
                      label="耗时"
                      value={fmtDuration(detailRun.elapsed_seconds)}
                  ...
                  {detailRun.started_at && (
                    <div className="text-xs text-text-muted flex gap-4 flex-wrap">
                      ...
                      {detailTokens && detailTokens.prompt + detailTokens.completion > 0 && (
                        <span>
                          Token: {(detailTokens.prompt + detailTokens.completion).toLocaleString()}
                        </span>
                      )}
                    </div>
                  )}
                ...
                <div className="p-6">
                  {(() => {
                    ...
                    if (!p)
                      return (
                        <div className="text-xs text-text-muted text-center py-4">暂无参数数据</div>
                      );
                    const fields: { label: string; value: string }[] = [
                      ...
                      {
                        label: '艺人 / 关键词',
                        value: Array.isArray(p.celebrities)
                          ? (p.celebrities as string[]).join(', ')
                          : String(p.celebrities || ''),
                      },
                      {
                        label: '搜索标签',
                        value: Array.isArray(p.search_tags)
                          ? (p.search_tags as string[]).join(', ')
                          : String(p.search_tags || ''),
                      },
                      {
                        label: '超话',
                        value: Array.isArray(p.super_topics)
                          ? (p.super_topics as string[]).join(', ')
                          : String(p.super_topics || ''),
                      },
                        ...
                        <tbody>
                          {fields.map((f, i) => (
                            <tr key={i} className="border-b border-border/30">
                              <td className="py-2 pr-4 text-text-muted whitespace-nowrap w-28 align-top">
                                {f.label}
                              </td>
                      ...
                      <tbody>
                        {detailEvents.map((evt, i) => {
                          ...
                          const displaySummary =
                            typeof summaryText === 'string' ? summaryText.slice(0, 80) : '';
                          const isExpanded = expandedEventRows.has(i);
                          const eventColor =
                            evt.event === 'run_started'
                              ? 'text-accent'
                              : evt.event === 'run_finished'
                                ? 'text-green-500'
                                : evt.event === 'step_complete'
                                  ? 'text-green-500'
                                  : evt.event === 'step_error'
                                    ? 'text-red-500'
                                    : evt.event === 'cancelled'
                                      ? 'text-red-500'
                                      : evt.event === 'agent_decision'
                                        ? 'text-yellow-500'
                                        : 'text-text-muted';
                          ...
                          return (
                            <>
                              ...
                              >
                                <td className="py-1.5 pr-2 text-text-muted/50 align-top">
                                  {i + 1}
                                </td>
            ...
            <div className="px-6 py-3 border-t border-border flex items-center justify-between">
              <button
                onClick={() => detailRun && handleDeleteRun(detailRun.run_id)}
                className="btn btn-ghost text-sm text-red-500 hover:bg-red-500/10"
              >
                <svg
                  className="w-3.5 h-3.5"
                  viewBox="0 0 24 24"
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => detailRun && handleContinueRun(detailRun)}
                ...
                <button onClick={closeDetail} className="btn btn-ghost text-sm">
                  关闭
                </button>
              </div>
            </div>
