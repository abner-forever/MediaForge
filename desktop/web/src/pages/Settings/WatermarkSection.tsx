    ...
    <div className="card space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-text-muted leading-relaxed pl-11">下载图片时自动检测并过滤含水印的图片。关闭后所有图片不做水印筛查直接保留。</p>
        </div>
        <div className="flex flex-col gap-1">
          <label className="toggle">
          ...
          <p className="text-xs text-text-muted leading-relaxed pl-11">过滤后若无水印图片数量不足，直接跳过该帖子，不进入发布流程。</p>
        </div>
        <div className="flex flex-col gap-1">
          <label>最少无水印图片数<NumberInput value={minClean} onChange={setMinClean} min={1} max={10} /></label>
          <p className="text-xs text-text-muted leading-relaxed">每篇帖子至少需要保留多少张无水印图片。配合严格模式使用——低于此数量时帖子会被跳过。</p>
        </div>
        <div className="flex flex-col gap-1">
          <label className="toggle">
          ...
          <p className="text-xs text-text-muted leading-relaxed pl-11">与严格模式搭配。开启后即使无水印图片不足最低数量，仍继续使用已有图片发布，而不是跳过帖子。</p>
        </div>
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-text-muted leading-relaxed">检测图片四角水印的灵敏度。值越小越容易被判定为有水印（更严格），值越大越宽松。默认 1.38。</p>
        </div>
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-text-muted leading-relaxed">检测图片底部横条水印的灵敏度。值越小越严格，值越大越宽松。默认 1.48。</p>
        </div>
      </div>
