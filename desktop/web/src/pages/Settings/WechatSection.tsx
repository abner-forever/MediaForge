...
export default function WechatSection({
  ...
  >({});
  const [deleteConfirm, setDeleteConfirm] = useState<{ accountId: string; name: string } | null>(
    null,
  );
    ...
    try {
      await wechatAccountApi.login(accountId, (evt: WeChatLoginEvent) => {
        if (evt.type === 'progress') {
          setLoginState((s) => ({
            ...s,
            [accountId]: { loading: true, message: evt.message || '' },
          }));
          ...
          setLoginState((s) => ({
            ...s,
            [accountId]: { loading: false, message: evt.message || '登录失败' },
          }));
  ...
  const [historyModal, setHistoryModal] = useState<{ accountId: string; name: string } | null>(
    null,
  );
  ...
  async function handleCheckStatus(accountId: string) {
    try {
      ...
      if (logged_in) {
        ...
        // 立即更新本地状态，再异步刷新全局
        setAccounts((prev) =>
          prev.map((a) => (a.account_id === accountId ? { ...a, logged_in: false } : a)),
        );
        await refreshAccounts();
      }
    } catch (err: any) {
  ...
  return (
    <div className="card space-y-4">
        ...
        <div className="space-y-2">
          {accounts.map((acc) => {
            ...
            return (
              <div
                key={acc.account_id}
                className="flex items-center gap-3 p-3 rounded-xl bg-bg-secondary border border-border"
              >
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${acc.logged_in ? 'bg-success' : 'bg-text-muted/40'}`}
                />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-muted">
                    ...
                    {acc.last_used
                      ? ` · 最后使用: ${new Date(acc.last_used).toLocaleString()}`
                      : ''}
                  </p>
                </div>
                {ls?.loading ? (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      className="btn btn-sm btn-ghost"
                      onClick={() => handleCheckStatus(acc.account_id)}
                      title="检测登录状态是否有效"
                    >
                      检测
                    </button>
                    {acc.logged_in ? (
                      <>
                        <button
                          ...
                          onClick={() => handleLogin(acc.account_id)}
                          title="重新登录"
                        >
                          重登
                        </button>
                        <button
                          className="btn btn-sm btn-ghost"
                          onClick={() => handleLogout(acc.account_id)}
                          title="清除登录态"
                        >
                          注销
                        </button>
                      ...
                      <button className="btn btn-sm" onClick={() => handleLogin(acc.account_id)}>
                        登录
                      </button>
                    )}
                    <button
                      className="btn btn-sm btn-ghost"
                      onClick={() => openHistory(acc.account_id, acc.name)}
                      title="发布历史"
                    >
                      历史
                    </button>
                    <button
                      ...
                      onClick={() => handleDelete(acc.account_id, acc.name)}
                    >
                      删除
                    </button>
                  </div>
      ...
      <Modal
        ...
        className="w-[640px] max-w-[calc(100vw-32px)] max-h-[80vh] overflow-y-auto"
      >
        ...
        {loadingHistory ? (
          ...
          <div className="space-y-2">
            ...
            {historyItems.map((item, i) => (
              <div
                key={`${item.id}-${i}`}
                className="grid grid-cols-[1fr_auto_auto] gap-3 items-center px-3 py-2 rounded-lg hover:bg-bg-secondary transition-colors"
              >
