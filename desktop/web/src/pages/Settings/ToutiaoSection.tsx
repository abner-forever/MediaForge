...
export default function ToutiaoSection({
  data,
  save,
  onReload,
}: {
      ...
      <div className="grid grid-cols-2 gap-4">
        <label className="col-span-2">
          ...
          <textarea
            ...
            placeholder={data.toutiao_cookie_set ? '已设置（留空保持不变）' : ''}
            rows={2}
          ...
          <div className="px-4 py-3 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-1 opacity-80 leading-relaxed">
                {verifyMessage || '请先通过头条快速登录获取 Cookie，或手动填写'}
              </p>
            </div>
          </div>
        </div>
          ...
          onClick={() =>
            withSave(async () => {
              const u: Record<string, string> = {
                TOUTIAO_COOKIE: rawCookie,
                TOUTIAO_UID: uid,
                TOUTIAO_SCREEN_NAME: screenName,
                TOUTIAO_AVATAR: avatar,
                TOUTIAO_USER_ID: userId,
                TOUTIAO_FETCH_MODE: fetchMode,
                TOUTIAO_SEARCH_TAGS: searchTags,
              };
              await save(u);
            })
          }
