...
export default function WeiboSection({
  data,
  save,
  onReload,
}: {
  ...
  const [verifyState, setVerifyState] = useState<'idle' | 'verifying' | 'valid' | 'invalid'>(
    'idle',
  );
    ...
    <div className="card space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {loginState === 'error' && <span className="text-xs text-danger">{loginMessage}</span>}
          {!rawCookie && (
            <button
      ...
      {screenName && (
        <div className="bg-accent-soft/40 border border-accent/20 rounded-xl px-4 py-3 flex items-center gap-3">
          {avatar && !avatarError ? (
            <img
              src={avatar.startsWith('http') ? `/proxy?url=${encodeURIComponent(avatar)}` : avatar}
              alt={screenName}
          ...
          <div className="text-xs text-accent font-medium bg-accent/10 px-2 py-1 rounded-lg">
            已登录
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <label className="col-span-2">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                className="text-xs text-text-muted hover:text-text-secondary px-1.5 py-0.5 rounded-lg hover:bg-bg-secondary transition-colors"
                onClick={() => setCookieRevealed(!cookieRevealed)}
              >
          ...
          <textarea
            value={cookieRevealed ? rawCookie : maskCookie('')}
            onChange={(e) => {
              setCookie(e.target.value);
              setCookieRevealed(true);
            }}
            placeholder={data.weibo_cookie_set ? '已设置（留空保持不变）' : ''}
            rows={2}
            className="font-mono text-xs"
            readOnly={!cookieRevealed && !cookie}
          />
        </label>
        <label>
          微博 UID
        ...
        <label>
          ...
          <input
            type="text"
            value={superTopics}
            onChange={(e) => setSuperTopics(e.target.value)}
            placeholder="迪丽热巴超话,杨幂超话（逗号分隔）"
          />
        </label>
        <label>
          场景标签
          ...
          <div className="px-4 py-3 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-1 opacity-80 leading-relaxed">
                {verifyMessage || '请先通过微博快速登录获取 Cookie，或手动填写'}
              </p>
            </div>
          </div>
        </div>
          ...
          onClick={() =>
            withSave(async () => {
              const u: Record<string, string> = {
                WEIBO_COOKIE: rawCookie,
                WEIBO_UID: uid,
                WEIBO_SCREEN_NAME: screenName,
                WEIBO_AVATAR: avatar,
                WEIBO_FETCH_MODE: fetchMode,
                WEIBO_CELEBRITIES: celebs,
                WEIBO_SEARCH_TAGS: tags,
                WEIBO_SCENE_EXTRA_TAGS: sceneTags,
                WEIBO_SUPER_TOPICS: superTopics,
              };
              await save(u);
            })
          }
          disabled={saving || loginState === 'loading'}
        >
          {saving ? (
            <>
