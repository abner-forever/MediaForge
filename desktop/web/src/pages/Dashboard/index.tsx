  ...
  async function load() {
    await withLoad(async () => {
      try {
        ...
        setDefaultAccount(
          accounts.accounts.find((a) => a.is_default) || accounts.accounts[0] || null,
        );
        setConnError(false);
      } catch {
        setConnError(true);
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="card p-4">
          ...
          <div className="text-base font-bold text-text">
            {defaultAccount?.name || '未设置账号'}
          </div>
          <div
            className={`text-sm mt-1 ${defaultAccount?.logged_in ? 'text-success' : 'text-danger'}`}
          >
            {defaultAccount?.logged_in ? '已登录，可用于发布' : '未登录，请先扫码'}
          </div>
          <button
            className="btn btn-sm mt-4"
            onClick={() => navigate({ pathname: '/settings', hash: 'system-wechat' })}
          >
            切换或登录
          </button>
        </div>
              ...
              recentQueue.map((item, i) => (
                <div key={item.id || i} className="text-sm">
                  ...
                  <div className="text-xs text-text-muted">
                    {item.images?.length || 0} 张图片 · {item.status || 'queued'}
                  </div>
                </div>
              ))
            ) : (
