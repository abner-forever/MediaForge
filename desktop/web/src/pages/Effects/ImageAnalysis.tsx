  ...
  return (
    <div className="card p-4">
      ...
      {loading ? (
        ...
        <div
          style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 160, padding: '0 4px' }}
        >
          {items.map((item, i) => {
            const h = (item.avg_reads / maxReads) * 130;
            return (
              <div
                key={item.image_count}
                style={{
                  flex: 1,
                  display: 'flex',
              ...
              >
                ...
                <div
                  style={{
                    ...
                    background:
                      i ===
                      items.reduce(
                        (bi, b, idx) => (b.avg_reads > items[bi].avg_reads ? idx : bi),
                        0,
                      )
                ...
                <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>({item.count}篇)</span>
              </div>
            );
          })}
        </div>
