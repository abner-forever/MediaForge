...
export default function Effects() {
  ...
  const handleSync = async () => {
    ...
    setSyncLogs(['正在连接...']);
    try {
      await wechatAccountApi.syncEffects(
        syncAccountId,
        (evt) => {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 32 }}>
          ...
          <HelpGuide title="数据分析 — 使用说明">
            <p>
              <b>1. 数据来源</b>
              ：在「发布队列」或「文章发布」页面对已发布内容录入效果数据（阅读量、点赞等），此处自动汇总展示。也可点击「同步公众号数据」自动从公众号后台抓取。
            </p>
            <p>
              <b>2. 概览卡片</b>：顶部 4
              个指标卡片展示发布总数、总阅读量、平均阅读和平均点赞，一眼掌握整体表现。
            </p>
            <p>
              <b>3. 趋势图</b>：折线图展示近 7/14/30 天的阅读量和点赞趋势，鼠标悬停查看每日详情。
            </p>
            <p>
              <b>4. 最佳时段</b>：热力图按「星期 ×
              小时」展示阅读量分布，深色区域代表高阅读量时段，帮你选择最佳发布时间。
            </p>
            <p>
              <b>5. 艺人排行</b>：按平均阅读量降序排列各艺人，快速识别最受欢迎的内容方向。
            </p>
            <p>
              <b>6. 多维对比</b>：按来源平台、内容类型、艺人三个维度对比阅读量，辅助内容策略决策。
            </p>
            <p>
              <b>7. 智能分析</b>：点击「生成分析」，AI
              将从内容策略、发布时段、互动率、图文配比等维度给出运营建议。
            </p>
            <p>
              <b>8. 导出数据</b>：点击「导出 CSV」下载完整数据表格，可用 Excel 做进一步分析。
            </p>
          </HelpGuide>
        </div>
      </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <input
                  ...
                  value={syncPages}
                  onChange={(e) =>
                    setSyncPages(Math.max(1, Math.min(50, Number(e.target.value) || 1)))
                  }
              ...
              <div style={{ flex: 1 }}>
                ...
                <input
                  ...
                  value={syncPageSize}
                  onChange={(e) =>
                    setSyncPageSize(Math.max(5, Math.min(50, Number(e.target.value) || 20)))
                  }
                  className="w-full text-sm text-center"
                  style={{ height: 36 }}
                  disabled={syncing}
