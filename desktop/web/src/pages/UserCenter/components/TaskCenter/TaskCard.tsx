      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between">
            ...
            <span className={`shrink-0 text-xs font-medium ${isDone ? 'text-green-400' : 'text-text-secondary'}`}>
              {isDone ? '已完成' : `${task.reward}`}
            </span>
          </div>
          <p className={`mt-1 text-xs leading-relaxed ${isDone ? 'text-green-400/60' : 'text-text-muted'}`}>
            {task.description}
          </p>
