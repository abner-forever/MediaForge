...
function InfoRow({
  ...
  return (
    <div className="flex justify-between items-center">
      ...
      <span className="text-sm text-text" style={valueColor ? { color: valueColor } : undefined}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <div className="card p-6">
      ...
      <div className="flex flex-col gap-3 mt-5 pt-5 border-t border-border">
        ...
        <InfoRow
          label="邮箱验证"
          value={user.is_verified ? '已验证' : '未验证'}
          valueColor={user.is_verified ? 'var(--success)' : 'var(--warning)'}
        />
      </div>
    </div>
  );
