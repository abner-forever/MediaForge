...
export default function CheckinCalendar({
  ...
  const calendarDays = useMemo(() => {
    ...
    const days: Array<{ day: number; date: string; checked: boolean; earned: number; streak: number; isToday: boolean; isFuture: boolean }> = []

    // 填充前置空白
    for (let i = 0; i < firstDay; i++) {
      days.push({ day: 0, date: '', checked: false, earned: 0, streak: 0, isToday: false, isFuture: false })
    }
      ...
      <div className="flex items-center justify-between mb-3">
        {checkinHistory && (
          <div className="flex gap-3 text-[11px] text-text-muted">
            <span>已签 <b className="text-accent">{checkinHistory.checked_days}</b> 天</span>
            <span>获得 <b className="text-success">{checkinHistory.total_earned}</b> 积分</span>
            <span>最长 <b className="text-warning">{checkinHistory.max_streak_in_month}</b> 天</span>
          </div>
        )}
      ...
      {historyLoading ? (
        ...
        <div className="grid grid-cols-7 gap-0.5">
          {calendarDays.map((item, index) => (
            ...
            >
              {item.day > 0 && (
                <span className={`text-[11px] ${item.isToday ? 'font-semibold' : 'font-normal'} ${item.checked || item.isToday ? 'text-accent' : 'text-text'}`}>
                  {item.day}{item.checked ? '✓' : ''}
                </span>
              )}
            </div>
          ))}
