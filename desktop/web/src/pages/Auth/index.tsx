...
import type { LoginRequest, LoginWithCodeRequest, RegisterRequest, ResetPasswordRequest } from '@/types'
...
export default function Auth() {
  ...
  const handleRegister = async () => {
    ...
    if (nickname.length < 1 || nickname.length > 20) { setError('昵称长度1-20位'); return }

    setSubmitting(true)
    setError(null)

    try {
      const request: RegisterRequest = { email, password, nickname, verification_code: verificationCode }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const request: ResetPasswordRequest = { email, verification_code: verificationCode, new_password: password }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (mode === 'login') {
      loginMethod === 'password' ? handlePasswordLogin() : handleCodeLogin()
        ...
        <div className="card" style={{ padding: '28px 24px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border)' }}>
                <button
                  ...
                  style={{
                    ...
                    borderBottom: loginMethod === 'password' ? '2px solid var(--accent)' : '2px solid transparent',
                    transition: 'all 0.2s',
                  }}
                >
                  密码登录
                ...
                <button
                  ...
                  style={{
                    ...
                    borderBottom: loginMethod === 'code' ? '2px solid var(--accent)' : '2px solid transparent',
                    transition: 'all 0.2s',
                  }}
                >
                  验证码登录
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)' }}>邮箱地址</label>
              <input
                type="email"
                autoComplete="email"
                required
            ...
            {mode === 'register' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)' }}>昵称</label>
                <input
                  type="text"
            ...
            {(mode === 'register' || mode === 'resetPassword' || loginMethod === 'code') && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)' }}>邮箱验证码</label>
              ...
              {mode === 'login' && (
                <>
                  ...
                  <button
                    ...
                    style={{
                      background: 'none', border: 'none',
                      color: 'var(--accent)', cursor: 'pointer', fontSize: 14, padding: 0,
                    }}
                  >
                    没有账号？立即注册
                  </button>
