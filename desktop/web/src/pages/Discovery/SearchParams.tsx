...
function SectionToggle({ open, onClick }: { open: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="p-0.5 rounded hover:bg-bg-secondary text-text-muted hover:text-text transition-colors shrink-0"
    >
      <svg
...
export default function SearchParams({
  ...
  tags: string;
  superTopics: string;
  toutiaoKeywords: string;
  filterWatermark: boolean;
  searching: boolean;
  platforms: Record<string, PlatformMeta>;
  activePlatform: PlatformMeta | undefined;
  ...
  setMinImages: (v: number) => void;
  setCelebs: (v: string) => void;
  setTags: (v: string) => void;
  setSuperTopics: (v: string) => void;
  setToutiaoKeywords: (v: string) => void;
  ...
  return (
    <div className="card space-y-4">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <svg
            className={`w-4 h-4 transition-colors ${showTarget ? 'text-accent' : 'text-text-muted'}`}
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
          ...
          <div className="space-y-2">
            ...
            {showToutiaoInput && (
              <div>
                ...
                <input
                  type="text"
                  className="input w-full"
                  value={toutiaoKeywords}
                  onChange={(e) => setToutiaoKeywords(e.target.value)}
            ...
            <div className="flex justify-start pt-0.5">
              <label className="toggle">
                <input
                  type="checkbox"
                  checked={filterWatermark}
                  onChange={(e) => setFilterWatermark(e.target.checked)}
                />
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            >
              {recommending ? (
                <>
                  <span className="w-3 h-3 border-2 border-text-muted/30 border-t-accent rounded-full animate-spin inline-block" />{' '}
                  分析中…
                </>
              ) : (
      ...
      <div className="flex items-center justify-between gap-3 pt-1 border-t border-border">
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            ...
            disabled={!selectedPosts.size || downloading}
          >
            {downloading ? (
              <>
              ...
              <>下载选中{selectedPosts.size > 0 ? ` (${selectedPosts.size})` : ''}</>
            )}
          </button>
          <button
            className="btn"
            onClick={onDownloadAll}
            disabled={!filteredIndices.length || downloading}
          >
            {downloading ? (
              <>
                <span className="w-3 h-3 border-2 border-text-muted/30 border-t-accent rounded-full animate-spin" />{' '}
