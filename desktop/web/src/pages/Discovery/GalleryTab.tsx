...
export default function GalleryTab({
  ...
  onSelectAllImages: (paths: string[]) => void;
            ...
            <div key={group.postIndex}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  ...
                  onClick={() =>
                    !allSelected
                      ? onSelectAllImages(groupPaths)
                      : groupPaths.forEach((p: string) => onToggleImageSelect(p))
                  }
                >
                  {allSelected ? '取消' : '全选'}
                </button>
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-2.5">
                {group.images.map((item: any) => {
                  ...
                  const scoreClass =
                    s.score >= 70 ? 'score-high' : s.score >= 40 ? 'score-mid' : 'score-low';
                    ...
                    >
                      <div className="relative">
                        <LazyImage
                          ...
                          onClick={() => {
                            const paths = allLocalImages.map((x: any) => lightboxSrc(x.path));
                            onOpenLightbox(
                              paths,
                              allLocalImages.findIndex((x: any) => x.path === item.path),
                            );
                          }}
                        />
                        <div
                          className={`absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 rounded-lg text-[11px] font-bold ${scoreClass}`}
                        >
