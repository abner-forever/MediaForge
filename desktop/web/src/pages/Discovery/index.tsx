...
export default function Discovery() {
  const {
    ...
    setProgress,
    recommendedCelebs,
    setRecommendedCelebs,
    discoveryCelebs: celebs,
    setDiscoveryCelebs: _setCelebs,
    discoveryTags: tags,
    setDiscoveryTags: _setTags,
    discoverySuperTopics: superTopics,
    setDiscoverySuperTopics: _setSuperTopics,
    discoveryToutiaoKeywords: toutiaoKeywords,
    setDiscoveryToutiaoKeywords: _setToutiaoKeywords,
  } = useStore(
    useShallow((s) => ({
  ...
  const setCelebs = useCallback(
    (v: string) => {
      markEdited();
      _setCelebs(v);
    },
    [markEdited, _setCelebs],
  );
  ...
  const setSuperTopics = useCallback(
    (v: string) => {
      markEdited();
      _setSuperTopics(v);
    },
    [markEdited, _setSuperTopics],
  );
  const setToutiaoKeywords = useCallback(
    (v: string) => {
      markEdited();
      _setToutiaoKeywords(v);
    },
    [markEdited, _setToutiaoKeywords],
  );
  ...
  useEffect(() => {
    Promise.all([platformApi.list(), settingsApi.get()])
      .then(([p, s]) => {
        setPlatforms(p.platforms);
        const def = p.default || 'weibo';
        if (p.platforms[def]) {
          setPlatform(def);
          setMode(p.platforms[def].default_fetch_mode);
        }
  ...
  const allLocalImages = useMemo(() => {
    const result: {
      path: string;
      scoreInfo: { score: number; reason: string; method: string };
      celebrity: string;
      scene: string;
      postIndex: number;
    }[] = [];
  ...
  async function doSearch(
    overrideCelebs?: string,
    overrides?: { tags?: string; superTopics?: string; toutiaoKeywords?: string },
  ) {
    setSearching(true);
      ...
      await searchStream(
        {
          platform,
          mode,
          celebrities: celebsToUse
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean),
          search_tags:
            platform === 'toutiao'
              ? toutiaoKeywordsToUse
                  .split(',')
                  .map((s) => s.trim())
                  .filter(Boolean)
              : tagsToUse
  ...
  async function loadMore() {
    ...
    setSearching(true);
    setSearchMessage(`正在加载第 ${nextPage} 页…`);
    const ctrl = new AbortController();
    searchAbortRef.current = ctrl;
    try {
      await searchStream(
    ...
    await withDownloading(async () => {
      ...
      try {
        await downloadStream(
          ...
          (evt) => {
            if (evt.type === 'start')
              setProgress({ current: 0, total: evt.total!, detail: '准备下载...' });
            else if (evt.type === 'progress')
              setProgress({
                current: evt.current!,
                total: evt.total!,
                detail: `${evt.celebrity} · ${evt.scene}`,
              });
            else if (evt.type === 'done') {
              discoveryApi.get().then((r) => {
                ...
                if (downloaded.length) {
                  ...
                  discoveryApi
                    .checkWatermark(downloaded)
                    .then((res) => setWatermarkedImages(new Set(res.watermarked)));
                }
              });
              addToast(
                `下载完成！${evt.downloaded} 张成功${evt.dropped ? `，${evt.dropped} 张跳过` : ''}`,
                'success',
              );
            }
          },
  ...
  async function doDownloadSelected() {
    if (!selectedPosts.size) {
      addToast('请先勾选要下载的帖子', 'error');
      return;
    }
    await doDownload([...selectedPosts].join(','));
    clearSelectedPosts();
  }
  ...
  async function doScore() {
    await withScoring(async () => {
      try {
        const r = await discoveryApi.score(true);
        setImageScores(r.scores);
        addToast(`评分完成！Vision: ${r.vision_count}，启发式: ${r.heuristic_count}`, 'success');
      } catch (err: any) {
        addToast(err.message, 'error');
      }
    });
  ...
  async function enqueueSelected() {
    if (!selectedImages.length) {
      addToast('请先选择图片', 'error');
      return;
    }
    await withEnqueuing(async () => {
      setProgress({ current: 0, total: 0, detail: '正在加入发布队列...' });
      try {
        const r = await queueApi.enqueueSelected(selectedImages);
        clearSelectedImages();
        addToast(`已加入发布队列：《${r.title}》`, 'success');
      } catch (err: any) {
        addToast(err.message, 'error');
      }
      setProgress(null);
  ...
  function handleSelectAllFiltered() {
    ...
    if (allSelected) {
      filteredIndices.forEach((i) => {
        if (selectedPosts.has(i)) togglePostSelect(i);
      });
    } else {
      filteredIndices.forEach((i) => {
        if (!selectedPosts.has(i)) togglePostSelect(i);
  ...
  async function handleAiRecommend() {
    ...
    try {
      ...
      addToast(
        recommendedCelebs.length ? '已刷新今日推荐' : '已推荐当前发文流量最大的明星，点击即可搜索',
        'success',
      );
    } catch (err: any) {
    ...
    <div className="py-6 px-4 max-w-[1280px] mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between">
        ...
        <HelpGuide title="图片发现 — 使用说明">
          <p>
            <b>1. 选择平台与模式</b>
            ：顶部选择内容来源（微博/头条等），以及搜索模式（按艺人、标签、关键词等）。
          </p>
          <p>
            <b>2. 设置筛选条件</b>
            ：输入艺人名或关键词，可设置页数和每页数量。开启「过滤水印」可自动跳过带水印的图片。
          </p>
          <p>
            <b>3. 搜索与浏览</b>
            ：点击「搜索」后结果以帖子卡片展示，可切换「帖子」和「图片墙」两种视图。
          </p>
          <p>
            <b>4. 选择图片</b>：勾选帖子或单张图片，底部操作栏可一键「AI
            评分」「下载到本地」「加入发布队列」。
          </p>
          <p>
            <b>5. AI 评分</b>：对选中图片调用 Vision API
            打分（0-100），按分数排序帮助筛选高质量图片。
          </p>
          <p>
            <b>6. 快捷操作</b>
            ：点击图片可放大预览，支持左右键翻页；长按帖子卡片可快速全选该帖所有图片。
          </p>
        </HelpGuide>
      </div>
      ...
      <SearchParams
        ...
        toutiaoKeywords={toutiaoKeywords}
        ...
        onSearch={() => doSearch()}
        onDownloadSelected={doDownloadSelected}
        onDownloadAll={() => doDownload(filteredIndices.join(','))}
        onScore={doScore}
        ...
        setCelebs={setCelebs}
        setTags={setTags}
        setSuperTopics={setSuperTopics}
        setToutiaoKeywords={setToutiaoKeywords}
        setFilterWatermark={setFilterWatermark}
        selectedPosts={selectedPosts}
        downloading={downloading}
        scoring={scoring}
        hasLocalAny={hasLocalAny}
        filteredIndices={filteredIndices}
        recommending={recommending}
        recommendedCelebs={recommendedCelebs}
        ...
        onSearchCeleb={(name) => {
          if (platform === 'weibo' && (mode === 'celebrities' || mode === 'mixed')) {
            ...
            const topic = `${name}超话`;
            setSuperTopics(topic);
            doSearch(undefined, { superTopics: topic });
          } else if (mode === 'keyword') {
            if (platform === 'toutiao') {
              setToutiaoKeywords(name);
              doSearch(undefined, { toutiaoKeywords: name });
            } else {
              setTags(name);
              doSearch(undefined, { tags: name });
            }
          }
        }}
      />
      ...
      {enqueuing && (
        <SearchLoadingOverlay
          title="正在加入发布队列"
          message={`已选 ${selectedImages.length} 张图片，正在处理…`}
        />
      )}
      ...
      <Dialog
        ...
        title="删除帖子"
        message={`确认删除第 ${removeConfirmIndex !== null ? removeConfirmIndex + 1 : ''} 条帖子？`}
        confirmText="删除"
        danger
        onConfirm={() => {
          if (removeConfirmIndex !== null) removePost(removeConfirmIndex);
