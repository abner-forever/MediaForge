...
FastAPI 60+ 端点：系统健康/统计、设置 CRUD、平台管理、微信多账号（SSE 登录）、发现搜索/下载/评分、文章 CRUD/AI 工具/发布、队列管理、素材文件夹 CRUD/元数据、日志、发布效果、合规查重。
...
### 模块划分
- `api/` — 按域拆分（base/sse + 14 个模块），`client.ts` 为 barrel re-export
- `types/` — TypeScript 类型按域拆分（11 个文件），`index.ts` 为 barrel
- `stores/` — Zustand slice 模式（9 个 slice），`index.ts` 组合导出 `useStore`
- `hooks/` — useLoading / useApi / useSSE / usePersistedState
- `components/` — `ui/`（原语）/ `layout/`（布局）/ `feature/`（业务），`index.ts` barrel
- `pages/` — 7 个页面目录（Dashboard / Discovery / Pipeline / ArticlePublish / Queue / Materials / Settings）
...
- `data/state/*.json` — 设置、文章、操作审计、素材元数据、各平台 auth
