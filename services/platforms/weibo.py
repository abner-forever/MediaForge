...
from config import WEIBO_UID_CACHE_PATH, settings
...
def _get_json_single(url: str, params: dict, *, referer: Optional[str] = None) -> dict:
    ...
    logger.info(
        "[DEBUG] API %s ok=%s keys=%s",
        resp.url,
        payload.get("ok") if isinstance(payload, dict) else "?",
...
def fetch_keyword_timeline_mobile(keyword: str, *, page: int, celebrity: str, scene: str) -> List[Dict]:
    ...
    statuses = payload.get("statuses", []) or payload.get("data", {}).get("list", []) or []
    ...
    for name in celeb_names:
        ...
        if uid:
            for pg in page_range:
                ...
                timeline_posts.extend(fetched)
                if progress_callback:
                    progress_callback(f"✓ {name} 时间线第{pg}页 → {len(fetched)} 条含图帖子")
...
def fetch_super_topic_posts(
    ...
    *,
    ...
    ajax/feed/topic 接口已失效（返回 404），改用关键词搜索
    「{topic_name}超话」获取超话相关帖子。
    """
    ...
    for page in pages:
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    f"✓ 超话「{topic_name}」第{page}页 → {sum(1 for s in (statuses or []) if _extract_images(s))} 条含图帖子"
                )
        except Exception as err:
            logger.error("超话「%s」第%s页抓取失败：%s", topic_name, page, err)
            if progress_callback:
                progress_callback(f"✗ 超话「{topic_name}」第{page}页失败：{err}")
        if page < max_pages:
    ...
    *,
    ...
    for tag in tags:
        ...
        for pg in pages:
            ...
            tag_posts.extend(fetched)
            if progress_callback:
                progress_callback(f"✓ 「{tag}」第{pg}页 → {len(fetched)} 条含图帖子")
            time.sleep(0.95 + pg * 0.05)
...
def fetch_weibo_posts_paginated(max_pages: int = 1) -> List[Dict]:
    """根据 settings.weibo_fetch_mode 分发到对应的抓取策略。"""
    ...
    if mode == "celebrities":
        ...
        if not celebrities:
            logger.warning("未配置 WEIBO_CELEBRITIES，celebrities 模式将回退为本人在线时间线")
            return finalize_posts(fetch_own_timeline_paginated(max_pages))
...
class WeiboService:
    meta = PlatformMeta(
        ...
        default_fetch_mode="celebrities",
        search_params_description="从微博搜寻明星美图，AI 智能评分筛选",
    )

    @staticmethod
    def check_auth() -> bool:
        return bool(settings.weibo_cookie)
